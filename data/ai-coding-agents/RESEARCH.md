...
### 12. Permission Management & Sandboxing

How the agent gates actions and isolates execution — the underlying permission and sandboxing systems that drive the day-to-day prompting experience and determine how safely it can run unsupervised. Finer-grained than the single `Permission Model` tag in Trust & Safety.

| Attribute | Type | Research Notes |
|-----------|------|----------------|
| **Sandboxing Technology** | tags | `os-sandbox`, `container`, `vm`, `filesystem-jail`, `none`. The actual isolation mechanism for command/tool execution. `os-sandbox` = kernel-level (macOS Seatbelt, Linux Landlock/seccomp, bubblewrap); `container` = Docker/devcontainer; `vm` = full VM or microVM; `filesystem-jail` = path-restricted writes with no kernel isolation; `none` = runs with the user's full privileges. Multi-select. |
| **Permission Granularity** | tags | `all-or-nothing`, `per-tool`, `per-command`, `command-prefix`, `per-path`, `per-domain`, `read-write-distinction`. How finely actions can be gated/allowlisted. `command-prefix` = allowlist rules like `git *` or `npm run *`; `per-domain` = network-host-level; `read-write-distinction` = separate gating for reading vs modifying. Multi-select. |
| **Rule Precedence** | tags | `none`, `allow-only`, `deny-overrides`, `nested-exceptions`, `ordered`. The most expressive precedence model the declarative rule engine supports. Single-pick the highest rung reached; check docs for sentences like "deny takes precedence over allow". |
| **Permission Configurability** | tags | `fixed`, `presets`, `declarative-rules`, `programmable`. How deeply the user can configure the permission engine — from a hardcoded policy to a programmable decision point (hooks/policy scripts). Single-pick the highest level. |
| **Sensitive File Protection** | boolean | Does the agent protect secrets (`.env`, key files) *by default*, without the user authoring a deny rule? Check default settings/docs for built-in deny patterns. |
| **Persistent Allowlist** | boolean | Can approvals be remembered (saved allow rules in config/settings) so the same action is not re-prompted across sessions? "Always allow this command" that survives a restart counts; a one-session-only remember does not. |
| **Unsupervised Mode** | boolean | Is there a documented full-autonomy mode that suppresses approval prompts (auto-approve, YOLO, `--dangerously-skip-permissions`, bypass-permissions) for hands-off runs? |
| **Network Access Control** | tags | `blocked`, `domain-allowlist`, `proxy`, `unrestricted`, `configurable`. How outbound network is gated during a run — particularly relevant for sandboxed/autonomous execution. |
| **Default Permission Posture** | tags | `read-only`, `prompt-each-action`, `auto-edit-ask-commands`, `auto-run-all`. What happens out of the box, before any configuration — the first-run developer experience. |
| **Auto-Accept Mode** | boolean | Does the agent offer a quickly-toggled, in-session intermediate autonomy mode that auto-applies edits (and optionally safe commands) without per-action prompts — like Claude Code's Shift+Tab "auto-accept edits"? Distinct from `Unsupervised Mode` (the full skip-all-permissions bypass). |
...
- **Sandboxing Technology**: Record the *actual* isolation mechanism, not a marketing "sandbox" label. `os-sandbox` only for genuine kernel enforcement (macOS Seatbelt, Linux Landlock/seccomp, bubblewrap). `container`/`vm` for Docker, devcontainers, or (micro)VMs — common for cloud/async agents. `filesystem-jail` when writes are confined to a path/workspace but commands still run with the user's privileges. `none` when the agent acts directly with full user privileges and relies on prompts alone. Multi-select when a tool offers several (e.g. local OS sandbox + cloud container).
- **Permission Granularity vs Permission Model**: `Permission Model` (Trust & Safety) is the coarse posture; this attribute is the fine detail. Tag every level the agent can gate at. `command-prefix` = pattern/prefix allowlists (`git *`, `npm run *`); `per-path` = write scope by directory; `per-domain` = network host allowlisting; `read-write-distinction` = reads and writes are gated separately (e.g. reads free inside the project, writes prompted). `all-or-nothing` only when approval is a single global switch with no scoping.
- **Rule Precedence**: Rate the *declarative* rule engine only — a `programmable` hook (see Permission Configurability) can always implement arbitrary precedence in code, but that must not inflate this value. Single-pick the most expressive model: `allow-only` = no deny rules at all; `deny-overrides` = deny rules exist but always win (an "allow A*, deny AB*" engine that cannot carve "allow ABC*" back out); `nested-exceptions` = specificity-based re-override, gitignore-style; `ordered` = user-controlled evaluation order (firewall-style first/last-match). `none` when there is no rule engine and every action prompts.
- **Permission Configurability**: Single-pick the highest level reached. `fixed` = built-in policy, at most a global on/off; `presets` = user picks among predefined modes/postures only; `declarative-rules` = user-authored allow/deny rules in config; `programmable` = a code-level decision point (hooks, policy scripts, permission callbacks) that can inspect the concrete action in context and allow, deny, or rewrite it. Orthogonal to `Rule Precedence`: configurability is *who can shape the policy and how*, precedence is *what the rule syntax can express*.
- **Sensitive File Protection**: `true` only for *default*, out-of-the-box gating of secrets (`.env`, key material) — shipped deny patterns or a built-in refusal. An engine where the user *could* write such a deny rule but none ships enabled is `false`; note the capability in the comment.
- **Persistent Allowlist**: `true` only if approvals are written to durable config (settings/rules file) and survive a restart — the mechanism that actually reduces repeat prompts. A remember-for-this-session toggle that resets on relaunch is `false`; note it in the comment.
- **Unsupervised Mode**: `true` for a documented switch that runs without approval prompts (`--dangerously-skip-permissions`, auto-approve, YOLO mode, full-auto). Cloud/async agents that are autonomous by design are `true`. An agent that always requires per-action approval with no bypass is `false`.
- **Network Access Control**: `blocked` = no egress by default; `domain-allowlist` = only approved hosts; `proxy` = routed through a controlled gateway; `unrestricted` = open network; `configurable` when the user sets the policy. Use `null` only when there is no credible signal about network handling.
- **Auto-Accept Mode**: `true` for a fast in-session toggle that stops the agent asking per action while still applying edits/commands (Claude Code's Shift+Tab "auto-accept edits", Cursor's auto-run, "accept all"). It is the *middle rung* between per-action prompting and full bypass: reserve `Unsupervised Mode` for the documented full skip-all-permissions/YOLO switch, and mark this one for the lighter, usually reversible in-session auto-apply. A tool whose only non-prompting option *is* the full bypass is `false` here (and `true` for Unsupervised Mode).
- **Default Permission Posture**: Describe the *unconfigured first-run* behavior, not what is achievable after tuning. `read-only` = cannot modify without opt-in; `prompt-each-action` = asks before every edit/command; `auto-edit-ask-commands` = applies file edits but asks before running commands; `auto-run-all` = executes edits and commands without asking.
...
- [x] Kimi CLI — Moonshot AI's open-source terminal agent with skills and MCP support; CLI/terminal form factor (added 2026-06-05)
